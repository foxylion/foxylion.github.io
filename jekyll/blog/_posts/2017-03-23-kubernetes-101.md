...
This is a pretty long article trying to cover an introduction to Kubernetes in dept. After mastering the article you should be able to manage a Kubernetes cluster, including secret management, rolling updates and zero downtime deployments. The table of contents should give an overview what else is contained.
...
Everything in this article is independent from the used infrastructure. It works locally, in the cloud or on your own infrastructure.
...
Kubernetes stores all the configuration and state of your cluster inside and [etcd store](https://github.com/coreos/etcd/tree/master/Documentation). This store is normally [highly available](https://github.com/coreos/etcd/blob/master/Documentation/op-guide/clustering.md). Otherwise your cluster can't reschedule any pods in case of an etcd outage.
...
Before you can use all those exciting Kubernetes concepts you need a running cluster. The Kubernetes provides multiple ways to create a cluster. It heavily depends on the environment where you are planning to run the cluster.

### Local

Most probably you would like to test all the concepts locally before you provision your cluster in the wild. Therefore exists a tool, it's called [minikube](https://github.com/kubernetes/minikube). They have a [guide](https://kubernetes.io/docs/getting-started-guides/minikube/) to get a local cluster up and running with literally one command. It is compatible with Linux, Windows and OS X. I would highly recommend you to use this for the first steps with Kubernetes.

### On Google Cloud

The Google Cloud provides a Kubernetes cluster out of the box, it is called [Google Container Engine](https://cloud.google.com/container-engine/). It's free when you use a small cluster with up to five nodes (you only pay for the compute engines). So this can be a good starting point to quickly get a running Kubernetes cluster in the cloud. A tutorial can be found [here](https://cloud.google.com/container-engine/docs/quickstart).

### On AWS

But also on AWS you can create a cluster quickly. It is not fully managed by AWS, but there are some tools which massively simplify the provisioning process. You can have a look at [kops](https://github.com/kubernetes/kops) and [here](https://github.com/kubernetes/kops/blob/master/docs/aws.md) is the quickstart guide.

### On your own hardware

If you are going to deploy Kubernetes on your own hardware, or another cloud which is currently not supported by automated tooling, you can use [kubeadm](https://kubernetes.io/docs/admin/kubeadm/) to create your own cluster. There is also a good [getting started guide](https://kubernetes.io/docs/getting-started-guides/kubeadm/) to create a cluster.

## Controll the cluster with `kubectl`
